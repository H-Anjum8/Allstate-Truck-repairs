...
export default function FleetTripPlanning() {
  ...
  const handleSelectLocation = item => {
    const place = item.address.freeformAddress;
    if (searchType === 'start') {
      setStart(place);
    } else {
  ...
  const { garageLat, garageLon, garageName, startLoc, destLoc } =
    route.params || {};
  const [start, setStart] = useState(startLoc || '');
  const [destination, setDestination] = useState(destLoc || '');
  ...
  const handleStartNavigation = () => {
    navigation.navigate('driver_selection', {
      start,
      destination,
      garageLat,
      garageLon,
  ...
  useEffect(() => {
    if (route.params?.selectedType && route.params?.selectedPlace) {
      if (route.params.selectedType === 'start') {
        setStart(route.params.selectedPlace);
        if (route.params.currentDest) setDestination(route.params.currentDest);
      } else if (route.params.selectedType === 'dest') {
        setDestination(route.params.selectedPlace);
        if (route.params.currentStart) setStart(route.params.currentStart);
      }
    }
  }, [route.params]);
  ...
  return (
    <SafeAreaView style={styles.container}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                onPress={() => handleSelectLocation(item)} // hides card automatically
              >
      ...
      <TouchableOpacity
        ...
        disabled={!(start && destination)} // 👈 disable until selected
      >
        <Text style={styles.addStopText}>Add Stops</Text>
      </TouchableOpacity>

      {/* Info Card (only if BOTH start & destination selected) */}
      {start && destination ? (
        <>
            ...
            {activeSummary ? (
              <>
                ...
                <InfoRow
                  ...
                  value={`${
                    activeSummary?.tollSummary?.highwayTolls || 0
                  } zones`}
                />
                <InfoRow
                ...
                <InfoRow
                  label="Route Type"
                  value={
                    activeSummary?.routeType || 'Highway / Trans-Canada Hwy'
                  }
                />
                <InfoRow
      ...
      >
        {loading ? (
          ...
          <Text style={styles.ctaText}>Suggest Driver</Text>
        )}
      </TouchableOpacity>
    </SafeAreaView>
  );
